"""
A complete implementation and training of a Caltech101 classifier.
...
def get_data() -> Tuple[DataLoader, DataLoader]:
    ...
    """
    # Add a 90 degree transpose to the data to make pretrained models not work:
    CONFIG.transforms.transforms.append(transforms.Lambda(lambda x: x.swapaxes(1, 2)))
    # Get the training data:
    # NO PRINTING!
    train_data = CIFAR10(root=ROOT_FILE, download=True, transform=CONFIG.transforms)
    # Create a data loader for the training data:
    train_loader = DataLoader(train_data, batch_size=CONFIG.batch_size, shuffle=True)
    # Get the test data:
    test_data = CIFAR10(root=ROOT_FILE, download=True, transform=CONFIG.transforms)
